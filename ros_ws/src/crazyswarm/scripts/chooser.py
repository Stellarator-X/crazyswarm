...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "../../../../crazyflie2-nrf-firmware/cf2_nrf.bin",
        ),
    ...
    if (
        not os.path.exists(os.path.join(args.configpath, "allCrazyflies.yaml"))
        or not os.path.exists(os.path.join(args.configpath, "crazyflieTypes.yaml"))
        or not os.path.exists(os.path.join(args.configpath, "crazyflies.yaml"))
    ):
        print(
            "ERROR: Could not find all yaml configuration files in configpath ({}).".format(
                args.configpath
            )
        )
        exit()
    ...
    def selected_cfs():
        nodes = [
            node for id, node in allCrazyflies.items() if widgets[id].checked.get()
        ]
        return nodes
    ...
    class CFWidget(Tkinter.Frame):
        def __init__(self, parent, name):
            ...
            checkbox = Tkinter.Checkbutton(
                self, variable=self.checked, command=save, padx=0, pady=0
            )
    ...
    def sysOff():
        ...
        for crazyflie in nodes:
            ...
            subprocess.call(
                ["rosrun crazyflie_tools reboot --uri " + uri + " --mode sysoff"],
                shell=True,
            )
    ...
    def flashSTM():
        ...
        for crazyflie in nodes:
            ...
            subprocess.call(
                [
                    "rosrun crazyflie_tools flash --uri "
                    + uri
                    + " --target stm32 --filename "
                    + args.stm32Fw
                ],
                shell=True,
            )
        ...
        for crazyflie in nodes:
            ...
            try:
                if not bigQuad:
                    ...
                    voltage = subprocess.check_output(
                        [
                            "rosrun crazyflie_tools battery --uri "
                            + uri
                            + " --external 1"
                        ],
                        shell=True,
